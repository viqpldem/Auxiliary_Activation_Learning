...
class Classifier(nn.Module):
    ...
    def __init__(self, cfg, n_labels, learning_rule, gcp, mesa):
        super().__init__()
        self.transformer = models.Transformer(cfg, learning_rule, gcp, mesa)
        self.top = Top_Classifier(cfg, n_labels)
        self.gcp = gcp
        self.mesa = mesa
...
class Top_Classifier(nn.Module):
...
def main(task='mrpc',
    ...
    model = Classifier(model_cfg, len(TaskDataset.labels), learning_rule, gcp, mesa)
    if mesa:
        ms.policy.convert_by_num_groups(model, model_cfg.n_heads)
        ms.policy.deploy_on_init(model, mesa_policy, verbose=print, override_verbose=False)
    ...
    if mode == 'train':
        def get_loss(model, batch, global_step): # make sure loss is a scalar tensor
            ...
            pre = torch.cuda.memory_allocated(1)
            logits = model(input_ids, segment_ids, input_mask)
            post = torch.cuda.memory_allocated(1)
